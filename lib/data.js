      ...
      .forEach(key => {
        ...
        if (Array.isArray(document[key])) {
          document[key].forEach(related_document => {
            relationships[key].push(get_payload(related_document, key))
          })
        }
        else if ((document[key]).constructor === Object) {
          relationships[key].push(get_payload(document[key], key))
        }
        else {
          relationships[key].push(get_payload({ id: document[key] }, key))
        }
      })
