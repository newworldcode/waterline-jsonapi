...
class Data_Payload_Generator {
  ...
  static resource_object(generator, document, no_relationships, is_included) {
    ...
    const out = {
      ...
      attributes: this.get_attributes(generator, document, is_included)
    }
    ...
    generator.associations
      .forEach(key => {
        ...
        if (Array.isArray(document[key])) {
          ...
          relationships[key].push(get_payload(document[key], related_identity))
        }
        else {
  ...
  static get_attributes(generator, values, is_included) {
      ...
      generator.associations
        .forEach(key => {
          if (object[key.toString()]) {
            object[key.toString()] = object[key.toString()].toString()
          }
        })
    }
