...
function payload(values, identity) {
  ...
  // If there's only one, just resolve to it.
  ...
  if (Array.isArray(vals)) {
    return vals.map(function(val) {
      return this.generate_payload_from_value(val, identity)
    }.bind(this))
  }
...
function relatives(to) {
  var discovered = relationships.discover(to, this)
  if (discovered) {
    to.relationships = discovered
  }
  return to
}
...
function toJSON() {
  if (this.is_error) {
    ...
    if (this.included_models) {
      out.included = this.included_models
    }
