...
class Data_Payload_Generator {
  ...
  static resource_object(generator, document) {
  ...
  static get_relationships(generator, document) {
    ...
    const get_payload = (doc, type) => ({
      ...
      links: {
        self: `${generator.api_root || ""}/${type}/${doc.id}`,
    ...
    generator.associations
      .forEach(key => {
        ...
        if (Array.isArray(document[key])) {
          ...
          get_payload(document, related_identity)
        }
        else {
          get_payload({ id: document[key] }, related_identity)
        }
      })
    ...
    Object.keys(relationships)
      .forEach(related_collection => {
        ...
        if (collection.is_array) {
          if (relationships[related_collection].length > 0) {
            return
          }
          // If it's empty, omit it.
          else {
            delete relationships[related_collection]
          }
        }
